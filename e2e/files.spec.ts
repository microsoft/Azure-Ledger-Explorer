...
test('Welcome hero shows all three import paths and How-it-works', async ({ page }) => {
  await page.goto('/files');
...
test('Signing Transparency card opens wizard pre-selected to the MST tab', async ({ page }) => {
  await page.goto('/files');
...
test('/mst-receipt renders the MST receipt verification page', async ({ page }) => {
  await page.goto('/mst-receipt');
  // The page should render rather than redirecting to /files.
  await expect(page).toHaveURL(/\/mst-receipt$/);
});
